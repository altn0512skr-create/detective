// ========================================
// パートナーカード（cards_p）
// ========================================

const partnerCardIds = [
  "P001","P002","P003","P004","P005","P006","P007","P008",
  "P009","P010","P011","P012","P013","P014","P015","P016",
  "P017","P018","P019","P020","P021","P022","P023","P024",
  "P025","P026","P027","P028","P029","P030","P031","P032",
  "P033","P034","P035","P036","P037","P038","P039","P040",
  "P041","P042","P043","P044","P047","P048","P049","P050",
  "P051","P052","P053","P054","P055","P056","P057","P058",
  "P059","P060","P061","P062","P063","P064","P065","P066",
  "P067","P068","P069","P070","P071","P072","P073","P074",
  "P075","P076","P077"
];

// ▼自動生成
const partnerCardList = partnerCardIds.map(id => ({
  id: id,
  image: `cards_p/${id}.png`,
  name: id,

  // パートナーは全部縦
  isVertical: true
}));